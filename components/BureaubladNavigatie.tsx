...
function NavItemMetKinderen({
  ...
  const jsAan = useClient();
  const submenuId = useId();
  const huidig = isNavigatieItemHuidig(pathname, item);
          ...
          {item.kinderen.map((kind) => (
            <li key={kind.href}>
              <Link
                href={kind.href}
                aria-current={
                  isNavigatieHuidig(pathname, kind.href) ? "page" : undefined
                }
  ...
  return (
    ...
    >
      <ul className="flex flex-nowrap items-center gap-5 text-[0.9375rem] xl:gap-8 xl:text-[1.0625rem] [&_li:last-child_.nav-submenu]:right-0 [&_li:last-child_.nav-submenu]:left-auto">
        {items.map((item) =>
          heeftKinderen(item) ? (
            <NavItemMetKinderen key={`${item.href}-${pathname}`} item={item} />
          ) : (
            <li key={item.href}>
