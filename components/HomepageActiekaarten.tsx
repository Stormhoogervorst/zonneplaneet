...
function SchuinePijl() {
  return (
    <svg
      ...
      viewBox="0 0 24 24"
      fill="none"
      className="ml-auto size-5 shrink-0 text-oranje-diep group-hover:text-navy"
    >
      <path
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {actieblokken.map(({ id, titel, beschrijving, href, Icoon }) => (
            <Link
              key={id}
              href={href}
              className="actie-keuzeblok group flex min-h-[104px] items-center gap-5 bg-keuzekaart p-6 transition-[background-color] duration-200 hover:bg-oranje motion-reduce:transition-none"
            >
              <span className="inline-flex size-14 shrink-0 items-center justify-center text-oranje group-hover:text-navy [&>svg]:size-14 [&>svg]:text-current">
                <Icoon />
              </span>
              <div className="min-w-0">
                <h3 className="text-[1.375rem] font-semibold text-navy group-hover:text-navy">
                  {titel}
                </h3>
